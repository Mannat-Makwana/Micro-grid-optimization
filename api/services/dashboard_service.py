...
def _build_forecast(frame: pd.DataFrame) -> dict:
    return {
        ...
        "horizon": len(frame),
        "start": _time(frame["timestamp"].iloc[0]),
        "end": _time(frame["timestamp"].iloc[-1]),
...
def _build_weather(frame: pd.DataFrame, source: str | None) -> dict:
    fields = {
...
def _build_history(frame: pd.DataFrame) -> dict:
    ...
    return {
        ...
        "start": _time(history["timestamp"].iloc[0]),
        "end": _time(history["timestamp"].iloc[-1]),
        "times": [_time_label(value) for value in history["timestamp"]],
        "timestamps": [_time(value) for value in history["timestamp"]],
...
def get_forecast_state() -> dict:
    ...
    forecast_data = _build_forecast(forecast)
    forecast_data["weather"] = _build_weather(weather_frame, weather_source)
    forecast_data["history"] = _build_history(history)
